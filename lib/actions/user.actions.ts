...
export const signUp = async ({ password, ...userData }: SignUpParams) => {
	// don't store password in create user table
	...
	try {
		...
		const newUser = await await database.createDocument(
			DATABASE_ID!,
			USER_COLLECTION_ID!,
			ID.unique(),
			{
...
export const createLinkToken = async (user: User) => {
	try {
		...
		console.error("Couldn't create plaid link token.", error);
	}
};
...
export const createBankAccount = async ({
	...
	try {
		...
		console.error("Couldn't create bank account. ", error);
	}
};
